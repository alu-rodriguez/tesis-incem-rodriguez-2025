...
pred blue_transition__S00__a__S01__mediante_met_constructor {
	some x: EstadoConcreto | partitionS00[x] and ((all sender:{this/Address}, flyer:{this/Address}, rewardsPerMile:{Int} | pre_params_constructor[x,sender, flyer, rewardsPerMile] implies some y:EstadoConcreto | met_constructor[x,y,sender, flyer, rewardsPerMile] and not partitionS01[y]))
}
run blue_transition__S00__a__S01__mediante_met_constructor for 10 EstadoConcreto, 3 Miles

pred blue_transition__S02__a__S02__mediante_met_addMiles {
	some x: EstadoConcreto | partitionS02[x] and (not pre_addMiles[x] or (all sender:{this/Address}, miles:{this/Miles} | pre_params_addMiles[x,sender, miles] implies some y:EstadoConcreto | met_addMiles[x,y,sender, miles] and not partitionS02[y]))
}
run blue_transition__S02__a__S02__mediante_met_addMiles for 10 EstadoConcreto, 3 Miles

pred blue_transition__S01__a__S02__mediante_met_addMiles {
	some x: EstadoConcreto | partitionS01[x] and (not pre_addMiles[x] or (all sender:{this/Address}, miles:{this/Miles} | pre_params_addMiles[x,sender, miles] implies some y:EstadoConcreto | met_addMiles[x,y,sender, miles] and not partitionS02[y]))
}
run blue_transition__S01__a__S02__mediante_met_addMiles for 10 EstadoConcreto, 3 Miles
