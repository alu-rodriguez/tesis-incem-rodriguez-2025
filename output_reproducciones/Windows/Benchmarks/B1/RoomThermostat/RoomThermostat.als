...
pred blue_transition__S02__a__S02__mediante_met_setTargetTemperature {
	some x: EstadoConcreto | partitionS02[x] and (not pre_setTargetTemperature[x] or (all sender:{this/Address}, targetTemperature:{Int} | pre_params_setTargetTemperature[x,sender, targetTemperature] implies some y:EstadoConcreto | met_setTargetTemperature[x,y,sender, targetTemperature] and not partitionS02[y]))
}
run blue_transition__S02__a__S02__mediante_met_setTargetTemperature for 10 EstadoConcreto

pred blue_transition__S01__a__S02__mediante_met_startThermostat {
	some x: EstadoConcreto | partitionS01[x] and (not pre_startThermostat[x] or (all sender:{this/Address} | pre_params_startThermostat[x,sender] implies some y:EstadoConcreto | met_startThermostat[x,y,sender] and not partitionS02[y]))
}
run blue_transition__S01__a__S02__mediante_met_startThermostat for 10 EstadoConcreto

pred blue_transition__S02__a__S02__mediante_met_setMode {
	some x: EstadoConcreto | partitionS02[x] and (not pre_setMode[x] or (all sender:{this/Address}, mode:{this/Mode} | pre_params_setMode[x,sender, mode] implies some y:EstadoConcreto | met_setMode[x,y,sender, mode] and not partitionS02[y]))
}
run blue_transition__S02__a__S02__mediante_met_setMode for 10 EstadoConcreto

pred blue_transition__S00__a__S01__mediante_met_constructor {
	some x: EstadoConcreto | partitionS00[x] and ((all sender:{this/Address}, thermostatInstaller:{this/Address}, thermostatUser:{this/Address} | pre_params_constructor[x,sender, thermostatInstaller, thermostatUser] implies some y:EstadoConcreto | met_constructor[x,y,sender, thermostatInstaller, thermostatUser] and not partitionS01[y]))
}
